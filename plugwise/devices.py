...
from plugwise.constant import ZONE_THERMOSTATS
...
class SmartEnergyMeter(DeviceBase):
    ...
    sensors: SmartEnergySensors = SmartEnergySensors()
...
class Zone(DeviceBase):
    ...
    sensors: ZoneSensors = ZoneSensors()
    thermostat: ThermostatDict = ThermostatDict()
    thermostats: ThermostatsDict = ThermostatsDict()
...
class Thermostat(DeviceBase):
    ...
    hardware: Optional[str] = None
    model_id: Optional[str] = None
    sensors: ThermostatSensors = ThermostatSensors()
    thermostat: ThermostatDict = ThermostatDict()
...
class ClimateDevice:
    ...
    sensors:  ClimateDeviceSensors = ClimateDeviceSensors()
    switches: ClimateDeviceSwitches = ClimateDeviceSwitches()
...
class Plug(DeviceBase):
    ...
    hardware: Optional[str] = None
    model_id: Optional[str] = None
    sensors: PlugSensors = PlugSensors()
    switches: PlugSwitches = PlugSwitches()
...
class PlugwiseData:
    ...
    def update_from_dict(self, data: dict[str, Any]) -> PlugwiseData:
        ...
        if data["device_class"] == "gateway":
            self.gateway.update_from_dict(data)
        if data["device_class"] == "heater_central":
            self.climate_device.update_from_dict(data)
        if data["device_class"] == "climate":
            self.zones.update_from_dict(data)
        if data["device_class"] in ZONE_THERMOSTATS:
            self.thermostats.update_from_dict(data)
        if data["device_class"].endswith("_plug"):
            self.plugs.update_from_dict(data)
        if data["device_class"] == "smartmeter":
            self.p1_dsmr.update_from_dict(data)
