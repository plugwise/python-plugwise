...
def power_data_energy_diff(
    measurement: str, net_string: str, f_val: float | int, direct_data: SmileSensors
) -> SmileSensors:
...
class SmileHelper:
    ...
    def __init__(self) -> None:
        ...
        self.gw_data: GatewayData = {"smile_name": "dummy", "gateway_id": None}
    ...
    def _add_appliance_data(self, d_id: str, device: DeviceData) -> DeviceData:
        ...
        if (
            ...
            if (lock_state := self._get_lock_state(appliance)) is not None:
                device["lock"] = lock_state
    ...
    def _power_data_peak_value(self, direct_data: SmileSensors, loc: Munch) -> Munch:
    ...
    def _power_data_from_location(self, loc_id: str) -> SmileSensors:
        """Helper-function for smile.py: _get_device_data().
        Collect the power-data based on Location ID, from LOCATIONS.
        """
        direct_data: SmileSensors = {}
        loc = Munch()
    ...
    def _get_lock_state(self, xml: etree) -> bool | None:
        """Helper-function for _add_appliance_data().
